...
2. 立项：`project new --title "X" --id <英文词> --profile book`（skill 自动派生 kn-book）
   → **只有确认全片零人物出镜时**才 `project set <id> --skip-design`。
   有人物（书中人物、常驻说书人 IP、任何反复出镜的具体形象）就走常规设定集：
   `skip_design` 关掉的不只是设定图，还关掉了唯一那条像素核对回路——
   `consistency scan` 会对每一镜返回 `reason: skip_design`、`verdict: null`
   （实测 kindling 十镜十个 null），全章画面从此无人核对。
...
## 视觉语言 DNA（题材在这里定，画风在 profile 里定）

profile 前缀只管**看起来什么样**（纸感光、低饱和暖色、油墨哑光层次），
它不规定画什么——画什么由本节与逐镜 `image_prompt` 定：

- **锚点镜（书卡/开场/收尾）**：书房暖光静物——精装书、台灯、咖啡、亚麻桌布，
  定全片调性。
- **内容镜（精华点）**：**优先把观点演成事件**。书里的案例本来就有人、有动作、
  有前后变化：把人登记进 `characters`、把取景地登记进 `scenes`、按镜绑定，
  才有得动、有得演、有得对剪。`characters` 整章全空的片子动不了也演不了，
  镜与镜之间没有任何可对剪的东西——讲焚书与阅读之死也能一个人都不出现。
  确实演不出来的抽象观点再退回概念隐喻插画（"注意力是资产"→
  `一枚金色沙漏立于书页翻开的山谷之间，沙粒是细小的金币`）——**隐喻必须物理自洽**：
  把「四五一度」与「烧书」硬焊成「冒火花的水银温度计」，温度计不冒火花，观众只看到
  一个不存在的物件。
- **肖像红线**：不画**真实可辨认的人**（作者本人、公众人物）——那是肖像权与
  相似度问题，用书桌/钢笔/背影剪影代替。书中人物、虚构角色、无从辨认的路人
  不在此列，照常登记进 `characters` 出镜。
