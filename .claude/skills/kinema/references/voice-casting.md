...
3. **定音色（声线描述 → 定制立档 → 启用，全系列零漂移）**：立项时给每个说话人写
   ...
   **配音表现力契约 `shots[].delivery{emphasis, pause_before, pause_after, note}`**（选填·全片基调
   ...
   · **句尾处理（引擎缺省，不用写）**：seed-audio 只在整段生成的末端截音、末音节的衰减会被切掉，
     ...
     拼旁白轨时每段配音尾部再淡出 70 ms。
   · **停顿的零点（读这条再写 `pause_*`）**：逐段 wav 落盘即把两端的数字静音归一到
     `voicecast.SPEECH_MARGIN`（0.12 s）——provider 回吐的音频头尾各带 0.2~0.6 s
     谁也没声明的余量，不摘掉它，你写的停顿就叠在一个浮动零点上（归一前实测：
     写 `pause_before: 0.6` 实际拿到 1.0 s，写 `pause_after: 0.1` 与不写毫无差别，
     被 `TAIL_ROLL` 的下限吞掉）。归一之后，kenburns 下每镜 `pause_after` 至少 0.25 s
     **就是「末音节到画面切点」的实际秒数**（写得更长照写，单侧上限 5 s）。
     这是**唯一能改剪辑出点节奏的旋钮**：不写则全片每一刀同相位（末音节 + 0.25 s），
     想让某一镜落定就把它写长，想接得紧凑就把余量留给下一镜的 `pause_before`。
     尾留白走与停顿同一道门控，dubbed/native 不加。
