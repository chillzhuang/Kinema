...
两条 todo 级检查，判据与阈值单一真源在 `pipeline/mediacheck.py`。

**片头起声（`head_silence`）**：片头 `HEAD_WINDOW`（0.4s）窗内**峰值** ≤
`HEAD_SILENT_DB`（−55 dB）即点名。取峰值不取均值——问的是「这半秒有没有一个听得见
的采样」，均值会被起声那一下拉上来。真片标定：静音开场 booktalk −78.3 / kindling
−72.2，有声开场 wandering −33.7 / pleats −21.2 / tianting −5.7，−55 落在两簇之间
20 dB 的无人区。

成因在**素材侧**：逐镜 TTS wav 自带起播静音、曲库 BGM 曲头自带静音加入轨 1.5s 淡入。
引擎不替人裁素材（裁 wav 会改 `dur`，整条时间轴与全部片段缓存跟着失效），故只点名，
由人决定改稿、换曲还是接受冷开场。

**接缝环境床（`seam_bed`）**：只在 native 且片段音轨作主轨时成立。量的是
`build/silent_<tag>.mp4`——**片段音轨母线本身**，不含旁白与 BGM；对成片本体量等于
把人声算进环境床，台阶会被人声占空比抹平（实测源头 31.7 dB 的落差在混过的成片上
只剩几 dB）。中间产物不在盘时记 info 跳过，不判坏片。

双条件才点名：落差 ≥ `SEAM_STEP_DB`（18 dB）**且**安静侧 ≤ `SEAM_FLOOR_DB`（−55 dB）。
只看落差会把「一句台词正好压着切点」误报成台阶（两侧都是有声电平）；加上绝对地板
之后，被点名的只剩「一边有房间、一边什么都没有」。

结论恒 todo 级：环境床是模型生成的内容，引擎只能名其失，不能凭空补一条床——那是发明
音频，不是确定性组装。出路是 retake 安静那一镜并在提示词里点名环境音，或给本章铺一条
作者提供的环境床。
