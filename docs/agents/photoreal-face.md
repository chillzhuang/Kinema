...
- **控制视频的复刻镜必须标 `face_visibility: closeup`**：不标就先走一轮路线 A，
  而写实档的分镜图是挂着设定图生的（图生图、不受信），那一轮注定被拒。标了直接从
  降级形态起步，省一次免费但要等的往返。
...
| V2V · 控制视频 | **同进阶梯，且恒不走 B** | 图本就挂 `role=reference_image`（不是首帧），官方互斥拦的是首/末帧与参考媒体混发，参考图 0~9 张与参考视频 0~3 段可以自由组合。降级形态＝场景基准图 + 身份图 + 控制视频，分镜图整个不进请求——这是写实人物复刻**唯一**发得出去的形态。板恒不挂：它与控制视频是两个并列的运动权威（同 previz 那条理由），而无板在这条路上不是缺口，控制视频逐帧给定走位与景别 |
| V2V · previz | 恒路线 A | previz 参考视频是无材质灰模，与取景地契约对撞；previz 镜不降级（运动预演与降级装配互斥） |
